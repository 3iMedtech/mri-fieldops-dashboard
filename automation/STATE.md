...
- **Snapshot timestamp:** 2026-06-08 — generated by `scripts/update-state.cjs`
...
- **Latest commit on `main`:** `e88a036` — chore: bump version to 1.6.4 for production fix release
- **Latest commit on `staging`:** `5499756` — fix: dbSaveEngineer no longer overwrites resignation status
- **Branch sync:** **diverged** — staging +10 / main +11 (see [[fieldops_prod_deploy_cherrypick]] deploy model)
- **Latest git tag:** `v1.6.1`
- **App version (`index.html`):** `1.6.1`
- **Working tree (when generated):** 16 uncommitted change(s)
